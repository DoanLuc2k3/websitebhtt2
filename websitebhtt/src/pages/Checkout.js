...
const Checkout = () => {
  ...
  // --- Tính toán giá (giữ nguyên) ---
  const subtotal = cartItems.reduce(
    (acc, item) => acc + item.product.price * item.quantity,
    0
  );
  const discount = passedState ? passedState.discountAmount : 0;
  const defaultBaseDeliveryFee = subtotal > 0 ? 20 : 0;
  const deliveryFee = passedState ? passedState.finalDeliveryFee : defaultBaseDeliveryFee;
  const total = subtotal + deliveryFee - discount;
  const discountLabel =
    passedState && passedState.appliedCouponName
      ? `Giảm giá (${passedState.appliedCouponName})`
      : "Giảm giá";
  const shippingLabel =
    passedState && passedState.appliedShippingRuleName
  ...
  const handleConfirmOrder = async () => {
    ...
    if (cartItems.length === 0) {
      message.warning("Không có sản phẩm trong giỏ hàng để thanh toán.");
      return;
    }
              ...
              renderItem={(item) => (
                <List.Item>
                  <List.Item.Meta
                    ...
                    title={<Text strong>{item.product.title} (x{item.quantity})</Text>}
                    description={`$${(item.product.price * item.quantity).toFixed(2)}`}
                  />
                </List.Item>
              )}
            />
